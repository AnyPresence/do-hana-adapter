...
A DataObject DataMapper connector for SAP HANA that uses ODBC to do its job. It is based on SQLServer DO code, but support for DataObjects is not complete due to the fact that things, such as regular expressions, are not currently supported in SAP HANA.
